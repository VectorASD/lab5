...
namespace Figurator.Models {
    public class Imager {
        ...
        private unsafe static void Dithering(BitmapData bd) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int ci = 0; ci < rgbs.Length; ci++) {
                    ...
                    // Манхэттенское расстояние в цветовом RGB-кубе:
                    // int dist = Math.Abs(Rd) + Math.Abs(Gd) + Math.Abs(Bd); // 0 .. 255*3
                    // Плохо
        ...
        private unsafe static NoiseDestroyerRes NoiseDestroyer(BitmapData bd, int threshold) {
            ...
            for (int i = 0; i < size; i++) {
                // Изначально тут было 2 цикла по x и y,
                // а позиция пикселя в стек кодировалась, не как x + w * y, а как x << 16 | y...
                // g_mat изначально был матрицей, а не массивом.
                // Это Вы сейчас видите УЖЕ оптимизированный до дыр вариант ;'-}
            ...
            /* for (int i = 0; i < size; i += w) Log.Write(Utils.Obj2json(g_mat[i..(i+w)]), true);
            Log.Write("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", true);
            int ii = 0;
            StringBuilder sb = new(); // Буквально ускорило в 50 раз, т.к. дозапись в файл маленькими кусочками слишком много времени съедает XD
            foreach (var shape in groups) sb.Append(ii++ + ":\ns: " + Utils.Obj2json(shape.shape) + "\nb: " + Utils.Obj2json(shape.border) + "\n");
            Log.Write(sb.ToString(), true); */
        ...
        /* public void Recurs(int x, int y, int depth = 0) {
            СЛИШКОМ МЕДЛЕННО!!!!!!!!!! Ещё на depth = 52104 зависает...
            Переделаю в обход в ширину XD
            Также пропадает смысл из метода делать класс из-за стека, по этой же причине
            я верну метод назад, что снова + скорость ;'-}

            if (g_mat[x, y] != 0) return;

            var c = data + (3 * (x + w * y));
            if (cur_c != (c[0] << 16 | c[1] << 8 | c[2])) return;

            Log.Write("xy: " + groups + " | " + x + " " + y + " | " + depth, true);
            g_mat[x, y] = groups;
            if (x > 0) Recurs(x - 1, y, depth + 1);
            if (y > 0) Recurs(x, y - 1, depth + 1);
            if (x + 1 < w) Recurs(x + 1, y, depth + 1);
            if (y + 1 < h) Recurs(x, y + 1, depth + 1);
        }*/
        ...
        public static void Import() {
            ...
            /* try {
                ...
                while (true) {
                    ...
                    Log.Write("Всего обнаружено групп: " + res.groups.Count);
                    Log.Write("  Из них подлежат стиранию: " + (res.yeahs + res.nops));
                    Log.Write("  Стёрты: " + res.yeahs);
                    Log.Write("  Не вышло: " + res.nops);
